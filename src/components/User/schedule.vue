<template>
  <div class="schedule" :style="{'height':`${domHeight}px`}">
    <scroller style="top:13vw;" :on-infinite="loadMore" :on-refresh="refresh" ref="myscroller">
      <div 
        class="schedule-item"
        @click="toDetail(item.id, item.sourceType)"
...
export default {
  ...
  methods: {
    toDetail(id, type) {
      if(type == 1) this.$router.push(`/activity/detail?id${id}`)
      else this.$router.push(`/curriculum/detail?id${id}`)
    },
    date(value) {
      // 日期
